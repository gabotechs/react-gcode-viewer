...
export default class StringReader implements BaseReader {
    ...
    private text: string = ""
    private opts: StringReaderOptions
    ...
    async read(): Promise<string | null> {
        if (!this.text) {
            this.text = typeof this.opts.file === "string" ? this.opts.file:(await this.opts.file.text())
        }

        if (this.cursor >= this.text.length) return Promise.resolve(null)
        const cursor = this.cursor
        this.cursor += this.opts.chunkSize || DEFAULT_CHUNK_SIZE
        return await new Promise<string | null>(res => setTimeout(
            () => res(this.text.slice(cursor, (this.opts.chunkSize || DEFAULT_CHUNK_SIZE)+cursor)),
